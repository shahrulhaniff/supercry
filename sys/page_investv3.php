...
<h1><?php if($PT=='MINI'){ echo "PRIME"; } else if($PT=='BASIC'){ echo "SUPER"; } else { echo "MEGA"; }?> INVESTMENT USING SCFX TOKEN</h1>
                </div>
            </div>
        ...
        <div class="col-sm-10 col-sm-offset-1 col-md-8 col-md-offset-2 col-lg-6 col-lg-offset-3">
            ...
            <div class="block">
                ...
                <div class="block-title">
                    <h2>INVEST (<?php if($PT=='MINI'){ echo "PRIME"; } else if($PT=='BASIC'){ echo "SUPER"; } else { echo "MEGA"; }?> PORTFOLIO)</h2>
                </div>
                <!-- END Form Validation Title -->
<?php 
/* value walletb(wallet balance) kita tukat jadi value walletb(token balance) */
$res=mysqli_query($conn,"SELECT tokenz from wallet WHERE id='".$_SESSION["USER"]."'");
$row=mysqli_fetch_row($res);
$walletb=$row[0];
//$walletb =  getWalletb($_SESSION['USER']);
$walletb=number_format((float)$walletb, 8, '.', '');
...
if($PT=='MINI'){
	...
	if($getTotalInvestToday==null){ $disabled = 'disabled'; }
	...
	$data3 = mysqli_query($conn,"SELECT * FROM masterctrl  ORDER BY created_date DESC LIMIT 1") or die(mysqli_error());
	$info3 = mysqli_fetch_array( $data3 );
	$minmodal = $info3['mininv'];
}
else { $minmodal = '99999'; }
?>
                <!-- Form Validation Form -->
						<?php  if($PT=='MINI'){ ?>
                <form id="form-validation" action="page_invest5.php" method="post" class="form-horizontal form-bordered">
				<?php } 
						else {
						?>
                <form id="form-validation" action="page_invest6.php" method="post" class="form-horizontal form-bordered">
				<?php } ?>
                    <div class="form-group">
                        <label class="col-md-3 control-label" >Token Balance</label>
                        <div class="col-md-6">
                    ...
                    <div class="form-group">
                        ...
                        <div class="col-md-6">
                            <input type="number" id="amountmini" name="amountmini" class="form-control floatNumberField" placeholder="SCFX" step="0.000000001" min="<?=$minmodal?>" max="<?=$maxi?>" step="0.01" autocomplete="off"  onChange="checkID(this.value)">
                        ...
                        <div class="col-md-6">
                            <input type="number" id="amount" name="amount" class="form-control floatNumberField" placeholder="SCFX" step="0.000000001" min="<?=$minmodal?>" max="<?=$maxi?>" step="0.01" autocomplete="off"  onChange="checkID(this.value)">
							<h5><span class="st" id="txtHint"></span></h5>
                        </div>
						
						<?php }

//if(($getTotalInvestToday==null)&&($PT=='MINI')){
